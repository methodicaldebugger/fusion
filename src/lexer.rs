...
impl Lexer {
            ...
            match ch {
                ...
                '\\' => {
                    ...
                    let escaped = match self.peek() {
                        ...
                        Some('\\') => '\\',
                        Some(other) => {
                            return self.error(format!("unknown escape sequence \\{}", other));
                        }
    ...
    fn read_number(&mut self) -> Result<Token, LexError> {
        ...
        while let Some(ch) = self.peek() {
            if ch.is_ascii_digit() {
                ...
                if has_dot {
                    return self.error("multiple decimal points in number literal");
                }
                has_dot = true;
                value.push(ch);
                self.advance();
    ...
    fn handle_indentation(&mut self) -> Result<(), LexError> {
        ...
        if width > current {
            ...
            self.pending_tokens
                .push_back(Spanned::new(Token::Indent, start, end));
                ...
                && width < *self.indentation_stack.last().unwrap()
            {
                ...
                self.pending_tokens
                    .push_back(Spanned::new(Token::Dedent, start, end));
            }
    ...
    fn next_spanned(&mut self) -> Result<Spanned<Token>, LexError> {
        loop {
            ...
            if self.line_start {
                ...
                while let Some(' ' | '\t') = self.input.get(look).copied() {
                    look += 1;
                }
            ...
            match ch {
                '\n' => {
                    ...
                    return Ok(Spanned::new(Token::NewLine, start, self.position));
                }
                '#' => {
                ...
                '/' if self.peek_next() == Some('/') => {
                    ...
                    self.advance();
                    if self.peek() == Some('/') {
                        return self.error("triple-slash comments are not supported");
                    }
                    while let Some(c) = self.peek() {
                        self.advance();
                        if c == '\n' {
                ...
                '/' if self.peek_next() == Some('*') => {
                    ...
                    self.advance();
                    if self.peek() == Some('*') {
                        return self.error("documentation comments are not supported");
                    }
                ...
                '}' => {
                    ...
                    return Ok(Spanned::new(Token::RightBrace, start, self.position));
                }
                ...
                '[' => {
                    self.advance();
                    return Ok(Spanned::new(Token::LeftBracket, start, self.position));
                }
                ']' => {
                    self.advance();
                    return Ok(Spanned::new(Token::RightBracket, start, self.position));
                }
                '(' => {
                    self.advance();
                    self.paren_depth += 1;

                    return Ok(Spanned::new(Token::LeftParen, start, self.position));
                }
                ')' => {
                    ...
                    return Ok(Spanned::new(Token::RightParen, start, self.position));
                }
                ',' => {
                    self.advance();
                    return Ok(Spanned::new(Token::Comma, start, self.position));
                }
                ':' => {
                    if self.peek_next() == Some(':') {
                        self.advance();
                        ...
                        return Ok(Spanned::new(Token::DoubleColon, start, self.position));
                    }
                    ...
                    return Ok(Spanned::new(Token::Colon, start, self.position));
                }
                '.' => {
                    self.advance();
                    ...
                    return Ok(Spanned::new(token, start, self.position));
                }
                '+' => {
                    self.advance();
                    return Ok(Spanned::new(Token::Plus, start, self.position));
                }
                '-' => {
                    self.advance();
                    let token = if self.peek() == Some('>') {
                ...
                '*' => {
                    self.advance();
                    return Ok(Spanned::new(Token::Star, start, self.position));
                }
                '/' => {
                    self.advance();
                    return Ok(Spanned::new(Token::Slash, start, self.position));
                }
                '=' => {
                    self.advance();
                    let token = if self.peek() == Some('=') {
                ...
                '&' => {
                    ...
                    if self.peek() == Some('&') {
                        self.advance();
                        return Ok(Spanned::new(Token::AndAnd, start, self.position));
                    }
                ...
                '|' => {
                    ...
                    if self.peek() == Some('|') {
                        self.advance();
                        return Ok(Spanned::new(Token::OrOr, start, self.position));
                    }
                    return self.error("expected `||`");
                }
                c if c.is_ascii_digit() => {
                    return Ok(Spanned::new(self.read_number()?, start, self.position));
                }
                c if c.is_alphabetic() || c == '_' => {
                    let ident = self.read_identifier();

                    if self.line_leading_keyword.is_none() && Self::is_block_keyword(&ident) {
